...
-  The wndr3800 I am using already had an earlier version of OpenWrt
   installed so this config does not document putting onto a new router
...
-  Two VLANs:
   ...
   -  10: Trusted - For development and other devices that I control the
      configuration on
   ...
   -  20: Untrusted - For devices that need network connection but are
      outside of my control (e.g. Roku Stick, guests, etc)
...
-  Separate NAT FW protection between...
   ...
   -  VLAN 20 <-> VLAN10 (VLAN10 can reach VLAN 20 devices but not vice
      versa))

-  Port security on VLAN 10 Lan ports to only allow configured mac
   addresses
...
* Update to latest OpenWrt[@Multiplef] [@Multipleh]
  :PROPERTIES:
  :CUSTOM_ID: update-to-latest-openwrtmultiplef-multipleh
  :END:

-  Download latest Barrier Breaker sysupgrade release

   -  openwrt-ar71xx-generic-wndr3800-squashfs-sysupgrade.bin from
...
-  *mistake*: I made the mistake of selecting to keep my configuration
   which soft bricked the router. Performed a Failsafe boot to
   recover[@Multipleh]

* VLAN10 setup
  :PROPERTIES:
  :CUSTOM_ID: vlan10-setup
  :END:

Set the internal VLAN10 network addresses so they will hopefully not
conflict with the external assigned address which is often private also.
...
* Lock down and update
  :PROPERTIES:
  :CUSTOM_ID: lock-down-and-update
  :END:
...
* Set Name and default domains
  :PROPERTIES:
  :CUSTOM_ID: set-name-and-default-domains
  :END:
...
* Allow keep to use wan
  :PROPERTIES:
  :CUSTOM_ID: allow-keep-to-use-wan
  :END:
...
* Attach 5Ghz radio to keep and bailey
  :PROPERTIES:
  :CUSTOM_ID: attach-5ghz-radio-to-keep-and-bailey
  :END:
...
* Setup Multiwan to support different WAN interfaces [@Multiplei]
  :PROPERTIES:
  :CUSTOM_ID: setup-multiwan-to-support-different-wan-interfaces-multiplei
  :END:
